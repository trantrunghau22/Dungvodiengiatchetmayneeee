...
class BoardScene:
        ...
        if self.app.ai_mode and not self.env.game_over and not self.popup_mode:
            if self.ai_agent:
                ...
                if self.ai_timer >= self.ai_delay:
                    ...
                    # 1. BACKUP (Để thử nước đi)
                    backup_board = self.env.board.copy()
                    backup_score = self.env.score
                    backup_game_over = self.env.game_over
                    current_ones = np.sum(self.env.board == 1)
                    
                    # 2. AI ĐÁNH GIÁ (Raw Q-Values)
                    q_values = self.ai_agent.act(self.env.board)
                    adjusted_q_values = q_values.copy()
                    
                    # 3. KÍCH HOẠT: "SÁT THỦ DIỆT SỐ 1"
                    # Kiểm tra cả 4 hướng, hướng nào giảm được số lượng số 1 thì ƯU TIÊN TUYỆT ĐỐI
                    for action in range(4):
                        # Reset
                        self.env.board = backup_board.copy()
                        self.env.score = backup_score
                        self.env.game_over = backup_game_over
                        
                        _, _, done, moved = self.env.step(action)
                        if moved and not done:
                            new_ones = np.sum(self.env.board == 1)
                            if new_ones < current_ones:
                                # Cộng điểm khổng lồ để AI bắt buộc phải đi
                                adjusted_q_values[action] += 1000000.0 
                    
                    # Reset lại để đi thật
                    self.env.board = backup_board.copy()
                    self.env.score = backup_score
                    self.env.game_over = backup_game_over
                    
                    # 4. SẮP XẾP NƯỚC ĐI (Từ Tốt -> Xấu)
                    sorted_actions = np.argsort(adjusted_q_values)[::-1]
                    
                    action_taken = False
                    
                    # 5. VÒNG LẶP AN TOÀN (Né nước đi tự sát)
                    for action in sorted_actions:
                        board, current_score, done, moved = self.env.step(action)
                        
                        if moved:
                            if not done:
                                # Đi được và SỐNG -> Chốt!
                                self.app.play_sfx('slide')
                                action_taken = True
                                break 
                            else:
                                # Đi xong CHẾT -> Undo ngay
                                self.env.board = backup_board.copy()
                                self.env.score = backup_score
                                self.env.game_over = backup_game_over
                                continue
                    ...
                    # 6. [CHỐNG ĐỨNG HÌNH] CHẾ ĐỘ CẢM TỬ
                    # Nếu tất cả hướng an toàn đều bị chặn -> Bắt buộc đi để kết thúc game hoặc tìm vận may
                    if not action_taken:
                        print("Không còn đường lùi! Kích hoạt chế độ Cảm Tử.")
                        for action in sorted_actions:
                            self.env.board = backup_board.copy()
                            self.env.score = backup_score
                            self.env.game_over = backup_game_over
    ...
    def render(self):
        ...
        self.screen.blit(score_surf, (340, 50)) 
        best_x = WWIDTH - 250 
        best_surf = self.score_font.render(str(self.env.top_score), True, (0,0,0))
        self.screen.blit(best_surf, (best_x, 50)) 
